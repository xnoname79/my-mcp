...
Shared HTTP server (streamable-http) hosting multiple tool sets:
  - sync-bridge: API spec synchronization between BE/FE sessions
  - unity-dev: Game development tools for Unity 3D (story, scenes, assets, scripts)
...
# ─── Unity Dev: Game Design Document ─────────────────────────────────────────
...
async def list_templates():
    ...
    for name, desc in descriptions.items():
        result.append({"template": name, "description": desc})
